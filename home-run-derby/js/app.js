...
(function(){
 ...
 function bracketOrder(scoreOrder){
  const hasRealSeparation=new Set(HRD.players.map(p=>Number(p.hr)||0)).size>1||HRD.players.some(p=>(Number(p.hr)||0)>0);
  if(hasRealSeparation)return {players:scoreOrder,label:'Live MLB totals'};
  const byName=new Map(HRD.players.map(p=>[p.name,p]));
  const projected=(HRD.projections||[]).map(r=>byName.get(r.name)).filter(Boolean);
  return {players:projected.length===HRD.players.length?projected:scoreOrder,label:'Monte Carlo projection'};
 }
 function renderSticky(leader){const root=document.getElementById('stickyLive');if(!root)return;const detail=[HRD.state.currentHitter&&`Hitting: ${HRD.state.currentHitter}`,HRD.state.timeRemaining&&`Time: ${HRD.state.timeRemaining}`,HRD.state.outsRemaining!=null&&`Outs: ${HRD.state.outsRemaining}`].filter(Boolean).join(' · ');root.innerHTML=`<span class="live-dot ${HRD.state.isLive?'':'idle'}"></span><div><strong>${HRD.state.isLive?'LIVE':'MLB'}</strong><small>${detail||HRD.state.eventStatus}</small></div><div class="sticky-leader"><small>Leader</small><strong>${leader?`${leader.owner} · ${leader.hr} HR`:'Waiting'}</strong></div>`}
 ...
 HRD.render=function(){
  ...
  const bracket=bracketOrder(s),adv=bracket.players.slice(0,4),m1=[adv[0],adv[3]],m2=[adv[1],adv[2]];
  document.getElementById('bracket').innerHTML=`<div class="sim-note">Seeds based on ${bracket.label}.</div><div class="match"><div class="match-title">Semifinal 1</div>${slot(m1[0],1)}${slot(m1[1],4)}</div><div class="match"><div class="match-title">Semifinal 2</div>${slot(m2[0],2)}${slot(m2[1],3)}</div><div class="match"><div class="match-title">Final</div><div class="slot"><span class="winner">Winner SF1</span><span>vs</span></div><div class="slot"><span class="winner">Winner SF2</span><span>TBD</span></div></div>`;
  ...
  renderSticky(s[0]);renderHighlights();
