...
(function(){
 ...
 function bracketOrder(scoreOrder){
  ...
  if(scoringStarted)return {players:scoreOrder,label:'live MLB totals'};
  const byName=new Map(HRD.players.map(p=>[p.name,p]));
  const projected=(HRD.projections||[]).map(r=>byName.get(r.name)).filter(Boolean);
  return {players:projected.length===HRD.players.length?projected:scoreOrder,label:'Monte Carlo odds before scoring'};
 }
 ...
 HRD.render=function(){
  ...
  document.getElementById('bracket').innerHTML=`<div class="sim-note">Seeds follow ${bracket.label}.</div><div class="match"><div class="match-title">Semifinal 1</div>${slot(m1[0],1)}${slot(m1[1],4)}</div><div class="match"><div class="match-title">Semifinal 2</div>${slot(m2[0],2)}${slot(m2[1],3)}</div><div class="match"><div class="match-title">Final</div><div class="slot"><span class="winner">Winner SF1</span><span>vs</span></div><div class="slot"><span class="winner">Winner SF2</span><span>TBD</span></div></div>`;
