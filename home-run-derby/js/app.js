...
(function(){
 ...
 HRD.render=function(){
  const s=sorted();
  document.getElementById('leaderboard').innerHTML=s.map((p,i)=>`<div class="player ${i===0?'leader':''}"><div class="rank">${i+1}</div><div><div class="name">${p.name}</div><div class="owner">${p.owner?'Owner: '+p.owner:'Owner not assigned'}</div></div><div class="score">${p.hr}<small>HOME RUNS</small></div></div>`).join('');
