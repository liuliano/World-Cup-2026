...
(function(){
 ...
 function playerStats(p){
  const stats=p.stats||HRD.emptyStats();
  return `<div class="stat-grid">${statLine('Max distance',stats.maxDistance==null?'—':`${fmt(stats.maxDistance)} ft`)}${statLine('Avg distance',stats.avgDistance==null?'—':`${fmt(stats.avgDistance)} ft`)}${statLine('Max exit velo',stats.maxExitVelocity==null?'—':`${fmt(stats.maxExitVelocity,1)} mph`)}${statLine('Avg exit velo',stats.avgExitVelocity==null?'—':`${fmt(stats.avgExitVelocity,1)} mph`)}${statLine(`Bonus HRs (${HRD.config.bonusDistance}+ ft)`,stats.bonusHomers||0)}${statLine('Tracked HRs',stats.trackedHomers||0)}</div>`;
 }
 ...
 HRD.render=function(){
  const s=sorted();
  document.getElementById('leaderboard').innerHTML=s.map((p,i)=>`<div class="player-card ${i===0?'leader':''}"><div class="player"><div class="rank">${i+1}</div><div><div class="name">${p.name}</div><div class="owner">${p.owner?'Owner: '+p.owner:'Owner not assigned'}</div></div><div class="score">${p.hr}<small>HOME RUNS</small></div></div>${playerStats(p)}</div>`).join('');
